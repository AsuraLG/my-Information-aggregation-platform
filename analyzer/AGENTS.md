...
| `ai_client.py` | AI 服务客户端封装：按 provider_type 统一调用 Anthropic / OpenAI 兼容接口 |
...
- AI 配置来自 `config/settings.yaml`，并支持 `INFO_AGG_AI_*` 环境变量兜底；`provider_type` 为必填，可选 `anthropic` / `openai`
...
### External
- `anthropic`、`openai` — AI 调用（支持 Anthropic / OpenAI 兼容 API）
